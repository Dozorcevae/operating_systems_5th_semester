...
static int parse_line(
		...
		char *type,
		unsigned *mode_oct,
		char rwx[0], char flags[32], char name[1024]){
	...
	*mode_oct = (unsigned)strtoul(f4, NULL, 8);
	...
	sprintf(rwx, 8, "%s", f5);
	sprintf(flags, 32, "%s", f6);
	sprintf(name, 1024, "%s", f7);
...
int main(void){
	...
	if (build(server_fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) die("bind");
		...
		while(1) {
			...
			*nl = '\0';
			...
			if (parse_line(p, &depth, &is_last, &type, &mode_oct, rwx, flags, name)) {
				...
				//Remember if current node is last (affects child prefix)
				last stack[depth] = is_last ? 1 : 0;
			}
