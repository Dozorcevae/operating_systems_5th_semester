...
static void fill_root_name(const char *path, char out[256]) {
    ...
    snprintf(out, 256, "%s", (base && base[0]) ? base : "home");
    sanitize_field(out);
}
...
int main(int argc, char **argv) {
    ...
    if (lstat(cfg.root, &st) != 0) {
        root.type = 'D';
        root.mode_oct = 0;
        strncpy(root.flags, "!", sizeof(root.flags) - 1);
        root.flags[sizeof(root.flags) - 1] = '\0';
    } else {
    ...
    fill_root_name(cfg.root, root.name);

    if (!record_send(sock, &root)) die("send root");
